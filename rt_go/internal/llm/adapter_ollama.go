...
func NewOllamaAdapter(cfg ports.LLMConfig) *OllamaAdapter {
	apiKey := cfg.APIKey
	if apiKey == "" {
		apiKey = "ollama"
	}
...
func (a *OllamaAdapter) Send(ctx interface{}, text string, tokenCh chan ports.LLMToken) error {
	if text == "" {
		return nil
	}
	...
	for {
		...
		if err != nil {
			...
			return fmt.Errorf("Ollama stream recv error: %w", err)
		}
...
func (a *OllamaAdapter) Stop() error {
	if a.cancel != nil {
		a.cancel()
	}
	return nil
...
func (a *OllamaAdapter) IsConnected() bool {
	return a.client != nil
}
