...
func NewDeepSeekAdapter(cfg ports.LLMConfig) *DeepSeekAdapter {
	baseURL := cfg.BaseURL
	if baseURL == "" {
...
func (a *DeepSeekAdapter) Send(ctx interface{}, text string, tokenCh chan ports.LLMToken) error {
	if text == "" {
		return nil
	}
	...
	for {
		...
		if err != nil {
			...
			return fmt.Errorf("DeepSeek stream recv error: %w", err)
		}
...
func (a *DeepSeekAdapter) Stop() error {
	if a.cancel != nil {
		a.cancel()
	}
	return nil
...
func (a *DeepSeekAdapter) IsConnected() bool {
	return a.client != nil
}
