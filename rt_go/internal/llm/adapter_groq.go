...
func NewGroqAdapter(cfg ports.LLMConfig) *GroqAdapter {
...
func (a *GroqAdapter) Send(ctx interface{}, text string, tokenCh chan ports.LLMToken) error {
	if text == "" {
		return nil
	}
	...
	for {
		...
		if err != nil {
			...
			return fmt.Errorf("Groq stream recv error: %w", err)
		}
...
func (a *GroqAdapter) IsConnected() bool {
	return a.client != nil
}
