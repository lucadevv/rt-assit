...
func NewAWSAdapter(cfg ports.LLMConfig) *AWSAdapter {
	region := cfg.Region
	if region == "" {
		region = "us-east-1"
	}
...
func (a *AWSAdapter) Send(ctx interface{}, text string, tokenCh chan ports.LLMToken) error {
	if text == "" {
		return nil
	}
	...
	if err != nil {
		return fmt.Errorf("AWS Bedrock stream error: %w", err)
	}
	defer stream.Close()
	...
	for {
		...
		if err != nil {
			...
			return fmt.Errorf("AWS Bedrock stream recv error: %w", err)
		}
...
func (a *AWSAdapter) IsConnected() bool {
	return a.client != nil
}
