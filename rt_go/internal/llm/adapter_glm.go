...
func NewGLMAdapter(cfg ports.LLMConfig) *GLMAdapter {
...
func (a *GLMAdapter) Send(ctx interface{}, text string, tokenCh chan ports.LLMToken) error {
	if text == "" {
		return nil
	}
	...
	if err != nil {
		return fmt.Errorf("GLM stream error: %w", err)
	}
	defer stream.Close()
	...
	for {
		...
		if err != nil {
			...
			return fmt.Errorf("GLM stream recv error: %w", err)
		}
...
func (a *GLMAdapter) IsConnected() bool {
	return a.client != nil
}
